...
public class BcSoapGenerator extends BcComponentGenerator {
  ...
  protected void generatePojo(final Data<?> definition) {
    ...
    definitionClass
      .annotate(javax.xml.bind.annotation.XmlAccessorType.class)
      .param("value", javax.xml.bind.annotation.XmlAccessType.FIELD);
    
    definitionClass
      .annotate(javax.xml.bind.annotation.XmlRootElement.class)
      .param("name", definition.getClassName());
    ...
    for (Data<?> attr : definition.getAttributes()) {
      ...
      // generate getters
      definitionClass
        .method(JMod.PUBLIC, attrField.type(), "get"+attr.getName())
        .body()._return(attrField);
  ...
  private JFieldVar generateAttribute(final JCodeModel codeModel, final JDefinedClass definedClass, final Class<?> attrClass, final String attrName, final Boolean isFinal) {
    return this.generateAttribute(codeModel, definedClass, codeModel.ref(attrClass), attrName, isFinal);
  }
  
  private JFieldVar generateAttribute(final JCodeModel codeModel, final JDefinedClass definedClass, final JClass attrClass, final String attrName, final Boolean isFinal) {
  ...
  private void buildOperation(final Operation operation, final JDefinedClass definedClass, final JCodeModel codeModel) {
    ...
    if(operation.getPostData() == null) {
      ...
      method = definedClass.method(JMod.PUBLIC, codeModel.directClass(operation.getPostData().getClassName()), operation.getScope().getName());
    }
    ...
    if (operation.getOperationType() == OperationType.ONE_WAY) {
      ...
      JInvocation responseBuilderInvocation = codeModel.ref(ResponseBuilder.class).staticInvoke("unmarshalObject")
          .arg(operation.getPostData().getMediaTypeAsString())
          .arg(serializedResponse)
  ...
  private void wrapOperationParams(JMethod method, JCodeModel codeModel, List<Data<?>> getDatas, JVar dataList) {
    ...
    for (Data<?> data : getDatas) {
      ...
      dataCreation
        ...
        .arg(JExpr.lit(data.isPrimitiveType()))
        .arg(JExpr.ref(data.getName()))
